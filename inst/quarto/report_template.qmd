...
if (!is.null(params$contrasts)) {
  if (params$generate_usageplots != "none") {
    if (params$generate_usageplots == "all") {
      features_to_plot <- list(
        ptm = hypothesis_tests$ptm$name,
        variant = hypothesis_tests$variant$name
      )
    } else {
      .signif_feat <- function(df) {
        df |>
          dplyr::filter(adjPval <= 0.05) |>
          dplyr::arrange(adjPval) |>
          dplyr::pull(name) |>
          unique()
      }
      features_to_plot <- list(
        ptm = .signif_feat(hypothesis_tests$ptm),
        variant = .signif_feat(hypothesis_tests$variant)
      )
    }
    names(features_to_plot) <- c(precursor_norm_name, "ptm", "variant")
    # TODO
    usagePlot(
      object = pe,
