...
xlabs <- purrr::map(aligned_histones$msa_ref, hptmUsage:::.mapper_from_msa) |>
  purrr::map(1) |>
  purrr::map2(aligned_histones$msa_ref, hptmUsage:::.fill_msa_gaps)
...
for (histone_family in histone_families) {
  ...
  p <- ggmsa::ggmsa(
    ...
    # Add the custom x-axis scale
    scale_x_continuous(
      position = "top",
      breaks = seq(1, length(xlabs[[histone_family]]), 5),
      labels = xlabs[[histone_family]][seq(1, length(xlabs[[histone_family]]), 5)],
      expand = c(0, 0)
    ) +
    theme(
    axis.text = element_text(size = 1000 / width(histones$msa[[histone_family]])[[1]]),
    axis.text.x.top = element_text(
      size = 750 / width(histones$msa[[histone_family]])[[1]],
      vjust = 0,
  ...
  ggplot2::ggsave(fs::path(quarto::get_running_project_root(), paste0("/out/floats/fig-", histone_family, "-msa"), ext = "svg"), plot = p)
...
p <- ggplot(
  tibble::as_tibble(colData(pe)[colData(pe)$group != "QC", ]),
...
p <- ggExtra::ggMarginal(
  p,
  mapping = aes(fill = colData(pe)[colData(pe)$group != "QC", ]$group),
  position = "stack",
  alpha = .6
)
ggplot2::ggsave(fs::path(quarto::get_running_project_root(), "/out/floats/fig-extraction", ext = "svg"), plot = p)
grid::grid.newpage()
grid::grid.draw(p)
```
...
# plot remaining
p <- rowData(pe[rowData(pe[["precursor"]])$histone, ][["precursor"]]) |>
  ggplot(aes(
    x = histone_family,
    fill = !is.na(mods_pep) &
  ...
  geom_bar() +
  labs(fill = "Modified")
ggplot2::ggsave(fs::path(quarto::get_running_project_root(), "/out/floats/fig-families", ext = "svg"), plot = p)
p
```
...
ggplot2::ggsave(fs::path(quarto::get_running_project_root(), "/out/floats/fig-abundance-ratio", ext = "svg"), plot = hist_abundance_qc)
hist_abundance_qc
```
...
# variant usage
if (is.null(params$usage_params[["usage_level"]])) {
  usage_level <- "histone"
} else {
  usage_level <- params$usage_params[["usage_level"]]
}
if (usage_level == "histone_group") {
  message("Variant usage cannot be normalized to the group itself (usage_level == 'histone_group') so this will be normalized to the histone family instead.")
  variant_usage_level <- "histone_family"
} else {
  variant_usage_level <- "usage_level"
}
...
model_designs[grepl("factor", names(model_designs), fixed = TRUE)] |>
  purrr::map("plotlist") |>
  purrr::iwalk(\(p, name) suppressMessages(ggplot2::ggsave(fs::path(quarto::get_running_project_root(), paste0("/out/floats/fig-design-", name), ext = "svg"), plot = p)))
...
if (all(is.null(params$random_effects))) {
  ...
  for (i in seq_along(model_designs)) {
    for (j in params$random_effects) {
    ...
    for (i in seq_along(formulas)) {
      ...
      pe <- pe |>
        ...
        hypothesisTest(
          ...
          resultsColumnNamePrefix = paste0(names(formulas)[[i]], "/"),
          overwrite = TRUE
        )
    }

    # extract hypothesis test results
    prefixed_contrasts <- purrr::imap(contrasts_full, \(x, idx) paste0(names(formulas)[[idx]], "/", x))
    hypothesis_tests[[viz_assay]] <- rowData(pe[[viz_assay]])[, unlist(prefixed_contrasts)]
    if (length(unlist(contrasts_full)) > 1) {
      hypothesis_tests[[viz_assay]] <- do.call(rbind, hypothesis_tests[[viz_assay]])
    }
    hypothesis_tests[[viz_assay]] <- hypothesis_tests[[viz_assay]] |>
      as_tibble() |>
      mutate(
        contrast = {
        ...
        formula = rep(names(formulas), times = purrr::map_int(contrasts, length) * nrow(pe[[viz_assay]])),
      )
...
if (!is.null(params$contrasts)) {
  ...
  for (viz_assay in c("co-extracts", "variant", precursor_norm_name, "ptm")) {
    for (formula_name in names(formulas)) {
      ...
      # save figures
      fs::dir_create(fs::path(quarto::get_running_project_root(), "/out/floats/volcano"))
      p <- volcanos[[viz_assay]][[formula_name]] +
        labs(
          x = bquote(log[2](FC)),
      ...
      ggplot2::ggsave(
        fs::path(
          quarto::get_running_project_root(),
          paste0(
            "/out/floats/volcano/", formula_name, "-",
            stringr::str_replace_all(assay_names[[viz_assay]], fixed(" "), "_"),
            "_volcano.pdf"
          ),
          ext = "svg"
        ),
...
```{r}
#| label: chunk-usageplots

if (!is.null(params$contrasts)) {
  if (params$generate_usageplots != "none") {
    usage_plots <- usagePlot(
      object = pe,
      assays = list(ptm = "ptm", variant = "variant"),
      model = purrr::set_names(params$design_formula, params$design_formula_names),
      significant_only = TRUE,
      show_legend = TRUE
    )
    # fix path length too long
    shorten_name <- function(name) {
      if (nchar(name) <= 150) return(name)
      parts <- stringr::str_split_1(name, "#")
      histone <- parts[[1]]
      if (length(parts) == 1) {
        # variants
        if (nchar(histone) > 100) {
          stringr::str_sub(histone, 98) <- "---"
        }
        return(histone)
      } else {
        # hPTMs
        mods <- parts[[2]]
        if (nchar(histone) > 50) {
          stringr::str_sub(histone, 51) <- "---"
        }
        if (nchar(histone) + 1 + nchar(mods) > 100) {
          stringr::str_sub(mods, 51) <- "---"
        }
        paste0(histone, "#", mods)
      }
    }
    usage_plots <- purrr::modify_depth(usage_plots, 2, function(x) {
      purrr::set_names(x, purrr::map_chr(names(x), shorten_name))
    })
    # store results
    purrr::iwalk(usage_plots$ptm, function(x, idx) {
      folder_name <- paste0("/out/floats/usage_plots/ptm_", idx)
      fs::dir_create(fs::path(quarto::get_running_project_root(), folder_name))
      purrr::iwalk(x, function(y, idx2) {
        suppressWarnings(suppressMessages(ggplot2::ggsave(
          fs::path(
            quarto::get_running_project_root(),
            folder_name,
            stringr::str_replace_all(idx2, c(
              " " = "_",
              "/" = "-",
              "#" = "-",
              "\\|" = ""
          ...
          plot = y,
          width = ncol(colData(pe)) * .5,
          scale = 1.5
        )))
      })
    })
    purrr::iwalk(usage_plots$variant, function(x, idx) {
      folder_name <- paste0("/out/floats/usage_plots/variant_", idx)
      fs::dir_create(fs::path(quarto::get_running_project_root(), folder_name))
      purrr::iwalk(x, function(y, idx2) {
        suppressWarnings(suppressMessages(ggplot2::ggsave(
          fs::path(
    ...
    print("Usage plots succesfully generated, see the 'Download figures' button below.")
  } else {
    print("No usage plots were generated, see the 'generate_usageplots' parameter.")
  }
} else {
  print("No contrasts defined!")
}
